...
document.querySelectorAll('a').forEach((link) => {
  if (!link.textContent.includes('Start Free Trial')) return;
  link.textContent = link.textContent.includes('→') ? 'Subscribe →' : 'Subscribe';
  link.href = 'register.html?source=subscription';
});
document.querySelectorAll('.pricing-grid a.button').forEach((link) => {
  link.textContent = 'Subscribe';
  const plan = link.closest('article')?.querySelector('.plan-name')?.textContent.trim().toLowerCase() || '';
  link.href = `register.html?source=subscription&plan=${encodeURIComponent(plan)}`;
});
...
async function loadPlatformPricing() {
  try {
    ...
    const cards = [...document.querySelectorAll('.pricing-grid article')];
    plans.slice(0, cards.length).forEach((plan, index) => {
      const card = cards[index];
      const monthly = Number(plan.price || 0);
      const annual = Math.round(monthly * 12 * .75);
      card.querySelector('.plan-name').textContent = plan.name.toUpperCase();
      const price = card.querySelector('.plan-price');
      price.dataset.monthly = monthly.toLocaleString('en-IN');
      price.dataset.annual = annual.toLocaleString('en-IN');
      price.innerHTML = `₹${monthly.toLocaleString('en-IN')}<small>/month</small>`;
      const limits = card.querySelectorAll('li');
      if (limits[0]) limits[0].textContent = `Up to ${Number(plan.memberLimit).toLocaleString('en-IN')} members`;
      if (limits[1]) limits[1].textContent = `Up to ${Number(plan.trainerLimit).toLocaleString('en-IN')} staff users`;
    });
  } catch (_) {}
}
loadPlatformPricing();
