...
function trainerAttendanceRecord(state, trainerId, date = todayDateKey()) {
  return state.trainerAttendanceLog.find((record) => record.trainerId === trainerId && record.date === date);
}
...
function renderTrainerAttendanceHistory(state, trainerId) {
  ...
  return `<div class="trainer-attendance-history"><h3>Recent attendance</h3><div class="attendance-history-table"><div class="attendance-history-head"><span>Date</span><span>Check in</span><span>Check out</span><span>Hours</span></div>${records.map((record) => {
    ...
    return `<div><span>${record.date}</span><strong>${record.time}</strong><strong>${record.checkOutTime || "—"}</strong><span>${hours}</span></div>`;
  }).join("")}</div></div>`;
}
...
async function downloadInvoice(state, invoice) {
  let logoDataUrl = "";
  try {
    const response = await fetch(`${apiBaseUrl}/assets/ai-gym-bodybuilding-logo-web.png`);
...
function setupRoleReferenceNavigation() {
  ...
  const showView = (view) => {
    ...
    panels.forEach((panel) => panel.classList.toggle("active", panel.dataset.rolePanel === view));
    window.scrollTo({ top: 0, behavior: "smooth" });
  };
  links.forEach((link) => {
